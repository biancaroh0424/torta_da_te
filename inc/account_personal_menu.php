<section class="product_filter">
        <div class="filter_header">
        <h3>Hi, Jane Doe</h3>
        <p>We are so grateful to having a special moments together!</p>
        </div>

        <div class="filter_body">
        <div>
            <a href="../torta_da_te/account_personal_info.php">
            <img src="../torta_da_te/images/myaccount.svg" alt="">
            <span>Personal Info</span>
            </a>
        </div>
        <div>
            <a href="">
            <img src="../torta_da_te/images/logout.svg" alt="">
            <span>Logout</span>
            </a>
        </div>
        <div>
            <a href="../torta_da_te/my_cart.php">
            <img src="../torta_da_te/images/cart_nav.svg" alt="">
            <span>My Cart</span>
            </a>
        </div>
        <div>
            <a href="../torta_da_te/account_mywishlist.php">
            <img src="../torta_da_te/images/wishlist.svg" alt="">
            <span>My Wishlist</span>
            </a>
        </div>
        <div>
            <a href="">
            <img src="../torta_da_te/images/myorder.svg" alt="">
            <span>My Order</span>
            </a>
        </div>
    </div>
    </section>